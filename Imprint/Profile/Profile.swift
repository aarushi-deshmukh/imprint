...
class Profile: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        Task {
            do {
                prof = try await NetworkManager.shared.fetchProfile()
                
                DispatchQueue.main.async {
                    self.profileView.reloadData()
                }
                
                print(prof) // now this prints correct data
            } catch {
    ...
    extension Profile: UICollectionViewDelegate, UICollectionViewDataSource {
        ...
        func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
            if indexPath.section == 0 {
                guard let cell = profileView.dequeueReusableCell(withReuseIdentifier: "info_cell", for: indexPath) as? ProfileInfoCell else {
                    return UICollectionViewCell()
                }
                guard let profile = prof.first else {
                    return cell
                }
                cell.configureInfo(with: profile)
                return cell
